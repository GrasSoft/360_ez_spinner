...
updater.addon = "addon_updater_spinwiz"
...
class Spinwiz_AddonUpdaterInstallPopup(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.manual_only:
            ...
            atr = Spinwiz_AddonUpdaterInstallPopup.bl_idname.split(".")
...
class Spinwiz_AddonUpdaterCheckNow(bpy.types.Operator):
...
class Spinwiz_AddonUpdaterUpdateNow(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if updater.update_ready:
            ...
            try:
                ...
                updater.print_trace()
                atr = Spinwiz_AddonUpdaterInstallManually.bl_idname.split(".")
                getattr(getattr(bpy.ops, atr[0]), atr[1])('INVOKE_DEFAULT')
        elif updater.update_ready is None:
...
class Spinwiz_AddonUpdaterUpdateTarget(bpy.types.Operator):
...
class Spinwiz_AddonUpdaterInstallManually(bpy.types.Operator):
...
class Spinwiz_AddonUpdaterUpdatedSuccessful(bpy.types.Operator):
...
class Spinwiz_AddonUpdaterRestoreBackup(bpy.types.Operator):
...
class Spinwiz_AddonUpdaterIgnore(bpy.types.Operator):
...
class Spinwiz_AddonUpdaterEndBackground(bpy.types.Operator):
...
def updater_run_success_popup_handler(scene):
    ...
    atr = Spinwiz_AddonUpdaterUpdatedSuccessful.bl_idname.split(".")
    getattr(getattr(bpy.ops, atr[0]), atr[1])('INVOKE_DEFAULT')
...
def update_notice_box_ui(self, context):
    ...
    colL.scale_y = 1.5
    colL.operator(Spinwiz_AddonUpdaterIgnore.bl_idname, icon="X", text="Ignore")
    colR = split.column(align=True)
    colR.scale_y = 1.5
    if not updater.manual_only:
        colR.operator(Spinwiz_AddonUpdaterUpdateNow.bl_idname,
                      text="Update", icon="LOOP_FORWARDS")
        col.operator("wm.url_open", text="Open website").url = updater.website
        # ops = col.operator("wm.url_open",text="Direct download")
        # ops.url=updater.update_link
        col.operator(Spinwiz_AddonUpdaterInstallManually.bl_idname,
...
def update_settings_ui(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        if "ssl" in updater.error_msg.lower():
            split.enabled = True
            split.operator(Spinwiz_AddonUpdaterInstallManually.bl_idname,
                           text=updater.error)
        else:
            split.enabled = False
            split.operator(Spinwiz_AddonUpdaterCheckNow.bl_idname,
                           text=updater.error)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(Spinwiz_AddonUpdaterCheckNow.bl_idname,
        ...
        col.scale_y = 2
        col.operator(Spinwiz_AddonUpdaterCheckNow.bl_idname)
        ...
        split.enabled = False
        split.scale_y = 2
        split.operator(Spinwiz_AddonUpdaterCheckNow.bl_idname, text="Checking...")
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(Spinwiz_AddonUpdaterEndBackground.bl_idname, text="", icon="X")
        ...
        split.operator(Spinwiz_AddonUpdaterUpdateNow.bl_idname, text=update_now_txt)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(Spinwiz_AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
        ...
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(Spinwiz_AddonUpdaterUpdateNow.bl_idname,
    ...
    if not updater.manual_only:
        ...
        if updater.include_branches and len(updater.include_branch_list) > 0:
            ...
            col.operator(Spinwiz_AddonUpdaterUpdateTarget.bl_idname,
        ...
        col.operator(Spinwiz_AddonUpdaterRestoreBackup.bl_idname, text=backup_text)
...
def update_settings_ui_condensed(self, context, element=None):
    ...
    if updater.error is not None:
        ...
        split.operator(Spinwiz_AddonUpdaterUpdateNow.bl_idname, text=now_txt)
        split = sub_col.split(align=True)
        split.scale_y = 2
        split.operator(Spinwiz_AddonUpdaterCheckNow.bl_idname,
                       text="", icon="FILE_REFRESH")
...
# -----------------------------------------------------------------------------
classes = (
    Spinwiz_AddonUpdaterInstallPopup,
    Spinwiz_AddonUpdaterCheckNow,
    Spinwiz_AddonUpdaterUpdateNow,
    Spinwiz_AddonUpdaterUpdateTarget,
    Spinwiz_AddonUpdaterInstallManually,
    Spinwiz_AddonUpdaterUpdatedSuccessful,
    Spinwiz_AddonUpdaterRestoreBackup,
    Spinwiz_AddonUpdaterIgnore,
    Spinwiz_AddonUpdaterEndBackground
)
...
def register(bl_info):
    ...
    updater.user = "GrasSoft"

    # Choose your own repository, must match git name for GitHUb and Bitbucket,
    # for GitLab use project ID (numbers only).
    updater.repo = "360_ez_spinner"

    # updater.addon = # define at top of module, MUST be done first

    # Website for manual addon download, optional but recommended to set.
    updater.website = "https://github.com/GrasSoft/360_ez_spinner/"
    ...
    updater.overwrite_patterns = ["*.png", "*.jpg", "README.md", "LICENSE.txt", "*.blend", "*.json"]
